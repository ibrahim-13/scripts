 ...
 * @returns {Promise<FloatingMenuCtrl>} Proxy to control floating menu
 */
...
`;
  ...
  return new Promise(resolve => {
    ...
    const init = () => {
      ...
      const elemCtxMenu = document.createElement('div');
      ...
      const menuState = new Proxy(state, {
        set(target, prop, value) {
          ...
          if (prop === 'actions') {
            ...
            renderContextMenu(); // Refresh with new actions
            return ret;
          }
      ...
      function showContextMenu(x, y) {
        elemCtxMenu.style.left = x + 'px';
        elemCtxMenu.style.top = y + 'px';
        elemCtxMenu.style.display = 'block';
      }

      // Build the context menu based on current actions
      function renderContextMenu() {
        elemCtxMenu.innerHTML = '';
        menuState.actions.forEach(action => {

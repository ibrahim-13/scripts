#!/usr/bin/env bash
#
# Test suite for sync2.sh.
#
# Creates all fixtures in a temporary sandbox (with an isolated $HOME so the
# real ~/.sync2-dirs is never touched), runs every scenario, and deletes
# everything when done — pass or fail.

set -u

SCRIPT="$(cd "$(dirname "$0")" && pwd)/sync2.sh"
SANDBOX="$(mktemp -d)"
trap 'rm -rf "$SANDBOX"' EXIT

export HOME="$SANDBOX/home"
CONFIG="$HOME/.sync2-dirs"
SRC="$SANDBOX/src"
DEST="$SANDBOX/dest"
...
setup_fixtures() {
    ...
    # matches deeper paths too. Known deviation, documented in sync2.sh.
    ...
    # sync2.sh translates them into explicit include rules scoped to the
...
check "config entry appended"       "grep -qxF '$SRC|$DEST' '$CONFIG'"
check "entry has exactly two fields (no ownership)" \
    "grep -qE '^[^|#]+\|[^|]+$' '$CONFIG'"
check "config records no uid:gid anywhere" \
    "! grep -qE '\|[0-9]+:[0-9]+' '$CONFIG'"
...
check "listing shows no ownership brackets" "! grep -qE '\[[0-9]+:[0-9]+\]' <<< \"\$out\"" 
check_out "reversed option is indented 2 extra spaces" '^   2\)'
check_out "reversed entry swaps direction"   "2\) \[$SANDBOX/\] dest -> src$"
...
# ---------------------------------------------------------------------------
out="$(run_sync 1 n)"
check_out "shows full command before confirm" 'rsync +-r +-l +-t +-v'
check_out "command has --delete"              '--delete'
check "command has no --chown"        "! grep -q -- '--chown' <<< \"\$out\""
check "command has no -a (archive)"   "! grep -qE ' -a( |vh)' <<< \"\$out\""
check "command has no -p/-o/-g"       "! grep -qE ' -[pog]( |$)' <<< \"\$out\""
check "command has no --perms/--owner/--group/--chmod" \
    "! grep -qE -- '--(perms|owner|group|chmod)' <<< \"\$out\"" 
...
# depth (git would anchor them). If this check ever flips, rsync/sync2.sh
# semantics changed — update the docs in sync2.sh accordingly.
...
# ---------------------------------------------------------------------------
echo "== Builtin directory walk (the 'find' replacement) =="
# ---------------------------------------------------------------------------
# sync2.sh collects .gitignore files with a recursive shell-builtin walk
# rather than 'find -print0', so the cases that walk has to handle itself are
# covered here: dot-directories, names containing spaces, deep recursion,
# symlinks, and pruning of globally-excluded directories. Each fixture pairs
# an exclude with a negation, since finding the .gitignore is what produces
# the negation's include rule — if the walk misses the file, the negated
# path silently stays excluded.
#
# Note on the symlink loop below: there is no portable 'timeout' (macOS has
# none), so the loop-safety property is asserted by the suite terminating at
# all. A walk that follows the loop hangs here instead of failing a check.
WSRC="$SANDBOX/wsrc"
WDEST="$SANDBOX/wdest"
rm -rf "$HOME" "$WSRC" "$WDEST"
mkdir -p "$HOME" "$WSRC"

# Dot-directory: reached only by the '.[!.]*' glob, not by '*'.
mkdir -p "$WSRC/.config"
printf '*.secret\n!keep.secret\n' > "$WSRC/.config/.gitignore"
echo drop > "$WSRC/.config/drop.secret"
echo keep > "$WSRC/.config/keep.secret"

# Directory name with a space: catches unquoted expansion in the walk.
mkdir -p "$WSRC/my docs"
printf '*.tmp\n!save.tmp\n' > "$WSRC/my docs/.gitignore"
echo drop > "$WSRC/my docs/drop.tmp"
echo save > "$WSRC/my docs/save.tmp"

# Five levels down: the walk recurses rather than scanning one level.
mkdir -p "$WSRC/a/b/c/d/e"
printf '*.deep\n!keep.deep\n' > "$WSRC/a/b/c/d/e/.gitignore"
echo drop > "$WSRC/a/b/c/d/e/drop.deep"
echo keep > "$WSRC/a/b/c/d/e/keep.deep"

# Symlinked directory (not descended, copied as a link) and a self-loop.
mkdir -p "$WSRC/real"
echo real > "$WSRC/real/file.txt"
ln -s "$WSRC/real" "$WSRC/link"
ln -s "$WSRC" "$WSRC/loop"

# .gitignore inside a globally-excluded directory: pruned by the walk, and
# its negation must not resurrect anything (layer 1 outranks layer 2).
mkdir -p "$WSRC/node_modules/pkg"
printf '!important.swp\n' > "$WSRC/node_modules/.gitignore"
echo dep > "$WSRC/node_modules/pkg/file.js"

"$SCRIPT" --init "$WSRC" "$WDEST" >/dev/null 2>&1
out="$(run_sync 1 y n)"
check_out "walk fixture sync completes" 'Sync complete'

check_file "dot-dir .gitignore found (negation applied)" "$WDEST/.config/keep.secret"
check_gone "dot-dir exclude applied"                     "$WDEST/.config/drop.secret"

check_file "spaced dir .gitignore found (negation applied)" "$WDEST/my docs/save.tmp"
check_gone "spaced dir exclude applied"                     "$WDEST/my docs/drop.tmp"

check_file "5-level-deep .gitignore found (negation applied)" \
    "$WDEST/a/b/c/d/e/keep.deep"
check_gone "5-level-deep exclude applied" "$WDEST/a/b/c/d/e/drop.deep"

check_file "real directory synced"          "$WDEST/real/file.txt"
check "symlinked dir copied as a symlink"   "[[ -L '$WDEST/link' ]]"
check "symlink loop copied as a symlink"    "[[ -L '$WDEST/loop' ]]"

check_gone "pruned node_modules still excluded" "$WDEST/node_modules"

# ---------------------------------------------------------------------------
echo "== No permission or ownership handling =="
# ---------------------------------------------------------------------------
# Modes and ownership are never read, recorded or applied. rsync runs with
# -rltv, so -a's implied -p/-o/-g are absent and the config carries no
# uid:gid columns.
out="$(run_sync 1 n)"
check_out "rsync runs with -r -l -t -v"  'rsync +-r +-l +-t +-v'
check "no --chown"        "! grep -q -- '--chown' <<< \"\$out\""
check "no -a (archive)"   "! grep -qE ' -a( |vh)' <<< \"\$out\""
check "no -p/-o/-g"       "! grep -qE ' -[pog]( |$)' <<< \"\$out\""
check "no --perms/--owner/--group/--chmod" \
    "! grep -qE -- '--(perms|owner|group|chmod)' <<< \"\$out\""
check "config stores no uid:gid" "! grep -qE '\|[0-9]+:[0-9]+' '$CONFIG'"

# A source mode the umask does not touch survives on a NEW file (that is
# rsync's behavior without -p: source mode masked by the receiver's umask),
# but a mode already on the destination is left alone even when the file is
# re-transferred. With -p the destination would be updated.
rm -rf "$HOME" "$WSRC" "$WDEST"
mkdir -p "$HOME" "$WSRC"
echo v1 > "$WSRC/f.txt"
chmod 644 "$WSRC/f.txt"
"$SCRIPT" --init "$WSRC" "$WDEST" >/dev/null 2>&1
run_sync 1 y n >/dev/null 2>&1
chmod 600 "$WDEST/f.txt"                       # destination mode diverges
chmod 755 "$WSRC/f.txt"                        # source mode changes
echo "v2-definitely-longer" > "$WSRC/f.txt"    # size change forces a transfer
run_sync 1 y n >/dev/null 2>&1
check "existing dest file is re-transferred" \
    "[[ \$(cat '$WDEST/f.txt') == v2-definitely-longer ]]"
check "existing dest mode left alone (no -p)" \
    "[[ \$(ls -l '$WDEST/f.txt' | cut -c1-10) == -rw------- ]]"
